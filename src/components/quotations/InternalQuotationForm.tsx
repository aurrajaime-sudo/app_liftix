...
import { Plus, Trash2, FileText, Calendar, Hash, AlertTriangle, Building2, X, DollarSign } from 'lucide-react';
...
interface InternalQuotationFormProps {
  onSuccess: () => void;
  onCancel: () => void;
}

export function InternalQuotationForm({ onSuccess, onCancel }: InternalQuotationFormProps) {
  const { user } = useAuth();
  ...
  const [items, setItems] = useState<QuotationItem[]>([
    { item_number: 1, description: '', quantity: 1, unit_price: 0 },
  ]);
  ...
  const removeItem = (index: number) => {
    if (items.length > 1) {
      ...
      setItems(newItems.map((item, idx) => ({ ...item, item_number: idx + 1 })));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (items.some((item) => !item.description || item.quantity <= 0)) {
      alert('Todos los items deben tener descripción y cantidad mayor a 0');
      return;
    }
